...
# Initial position/motion of the rolling element
r = (c0 + b0 - d0 )/2;  # Radius
th = -pi/4.; # Angle (theta)
rdot = 0;   # Derivative of radius
thdot = (2.*a0 + 3.*b0 + c0 + d0)/4./(b0+c0)*w0; # Angular speed
##phi = 0;  # Rotation angle of RE around its own axis
phidot = (b0 + c0 - d0)*(2.*a0 + 3.*b0 + c0 + d0)/2./(b0+c0)/(2.*a0 + b0 - c0 - d0)*w0; # Derivative of phi
...
y0 = array([r, th, rdot, thdot, phidot])

def oderhs2(t,y):
    return oderhs(y,t)


(Y, info) = lsodar.odeintr(oderhs, y0, T, atol = 1e-8, rtol = 1e-8, full_output=1)
